...
# --- Import SOTA pipeline libraries ---
import random
import torch
...
# --- Check for .png files again ---
raw = Path('data/raw')
files = sorted(raw.glob('*.png'))
print(f"Step 2: Found {len(files)} .png files in {raw}")
if len(files) < 2:
    raise Exception("❌ ERROR: Not enough .png files. Did the previous cell finish successfully?")

# --- Split train/test (unsupervised, so random split) ---
...
print("Step 2: Loaded DINOv2-base model.")

# --- Feature extraction from normal patches ---
features = []
with torch.no_grad():
...
# --- Fit multivariate normal on a feature subset (PaDiM) ---
...
# --- Show top 12 most anomalous patches ---
...
src = Path('/content/drive/MyDrive/synthetic_dataset_anomalies')
npy_files = sorted(src.glob('*.npy'))
print(f"Now found {len(npy_files)} .npy files in {src}")
if npy_files:
    print("First 3 files:", npy_files[:3])
...
print(f"Step 1 result: {converted} images converted. {len(png_files)} .png files now in {raw}")

# === SOTA DINOv2 + Mahalanobis Anomaly Detection ===
import random
import torch
import numpy as np
from pathlib import Path
...
print(f"Step 2: Found {len(files)} .png files in {raw}")
if len(files) < 2:
    raise Exception("❌ ERROR: Not enough .png files for anomaly detection.")
...
# ---- 6. LOAD PRETRAINED DINOv2 MODEL ----
model = AutoModel.from_pretrained("facebook/dinov2-base").to(device).eval()
print("Loaded DINOv2-base model.")

# ---- 7. FEATURE EXTRACTION FROM NORMAL PATCHES ----
features = []
with torch.no_grad():
    for pixels, _ in train_dl:
        f = model(pixel_values=pixels.to(device)).last_hidden_state[:,0,:].cpu().numpy()
...
# ---- 8. FIT MULTIVARIATE NORMAL ON FEATURE SUBSET (PaDiM) ----
sel = np.random.choice(features.shape[1], PADIM_DIM, replace=False)
mu = features[:,sel].mean(axis=0)
cov = np.cov(features[:,sel], rowvar=False) + np.eye(PADIM_DIM)*1e-6
icov = np.linalg.inv(cov)
...
# ---- 11. SHOW TOP 12 MOST ANOMALOUS PATCHES ----
topk = np.argsort(patch_scores)[-12:][::-1]
fig, axs = plt.subplots(3, 4, figsize=(12,8))
for ax, idx in zip(axs.flat, topk):
    im = Image.open(file_names[idx])
